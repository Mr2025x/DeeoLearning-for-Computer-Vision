...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      ...
      dists[i] = torch.sum(diff * diff, dim = (1, 2, 3))
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_sum = torch.sum(x_train_flat * x_train_flat, dim = 1)
    x_test_sum = torch.sum(x_test_flat * x_test_flat, dim = 1)
    cross = x_train_flat @ x_test_flat.T
    
    dists = x_train_sum.reshape(-1, 1) + x_test_sum.reshape(1, -1) - 2 * cross
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      _, nearst_indices = torch.topk(dists[:, i], k = k, largest = False)
      nearest_labels = y_train[nearst_indices]
      votes = torch.bincount(nearest_labels, minlength = 10)
    ...
    for k in k_choices:
      ...
      for fold in range(num_folds):
        x_train_fold = torch.cat(
          [x_train_folds[i] for i in range(num_folds) if i != fold],
          dim = 0
        )
        y_train_fold = torch.cat(
          [y_train_folds[i] for i in range(num_folds) if i != fold],
          dim = 0
        )
        ...
        accurcy = classifier.check_accuracy(x_val_fold, y_val_fold, k=k, quiet=True)

        k_to_accuracies[k].append(accurcy)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
