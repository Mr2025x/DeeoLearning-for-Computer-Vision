...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    row_indices = torch.tensor([1, 0, 3])
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    y = torch.zeros((len(x), 1 + x_tensor.max()), dtype=torch.float32)
    y.scatter_(1, x_tensor.unsqueeze(1), 1)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = x.reshape(4, 6).transpose(1, 0).reshape(3, 8)
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = torch.where(x == x.min(dim=1, keepdim=True)[0], torch.zeros_like(x), x)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    M = x.shape[0]
    col_mean = x.sum(dim=0) / M
    y = (x - col_mean) / (((x - col_mean) ** 2).sum(dim=0) / (M - 1)) ** 0.5
